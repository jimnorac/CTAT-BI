# CTAT-History.py    SKY 11 In=Z Out=E    Version 2022-12-4     Add break to try
...
header_names = ['Callsign','Date','Time','UTC_Date','UTC_Time','Category','Type','Msg','Rules','Airport','Alt','Bcn','Scr1','Scr2','ADE','Position','APO','Twr','Rdr','Area',\
                'Airspace','Entry','Exit','Hdg','Spd','Runway','WC','TA','X','Y','Fnct','S1','S2','S3','F1','F2','X2','AA','CM',"Year","Month","Day","Hour","CSAlpha","Beacon"]
...
print(f"InPath > {DriveIn}:\FTP\AcHistory     OutPath > {DriveOut}:\CTAT_BI")
...
while i < len(site):  
  ...
  out_path = DriveOut+":/CTAT_BI/History/{}/".format(site[i])         # Output data path  
  ...
  for in_file in os.listdir():                                        # eg FAR_CountOps_AcHistory_2022-09-06_Lcl.csv  s
    ...
    base_date = modified_date                                         # account for loading base_date using either file modified data or date stripped from file name
    if base_name == "Date in the File Name":                          # switch between date in file name and file modified date   base_name set at top of script
      base_date = file_date    
    if base_date >= back_date:                                        # Continue when file or modified date is not more that b_days before current date  
      with open(in_file, 'r') as csv_file:                          # read all Folder files that pass date check
          ...
          df['CSAlpha'] = df['Callsign'].apply(get_alpha)             #                 Leading Aloha from callsign
          ...
          zfilter= (df['Twr']=='A') | (df['Twr']== 'C') | (df['Twr'] == 'E') | (df['Twr']=='F') | (df['Twr']== 'G') | \
                   (df['Rdr']=='A') | (df['Rdr']== 'B') | (df['Rdr'] == 'C') | (df['Rdr']=='E') | (df['Rdr']== 'G') | (df['Rdr']=='H') | (df['Rdr']== 'I') | (df['Rdr'] == 'K') | \
                   (df['Rdr']=='M') | (df['Rdr']=='O')| \
                   (df['Area']=='A') | (df['Area']== 'B') | (df['Area'] == 'C') | (df['Area']=='E') | (df['Area']== 'G') | (df['Area']=='H') | (df['Area']== 'I') | (df['Area'] == 'K') | \
                   (df['Area']=='M') | (df['Area']=='O')
          df1 = df[zfilter]                                           # keep only rows where at least one column among Twr Rdr or Area is not Null
          if len(df1) >= 1:                                           # ouput only create file that have data beyond the header some days with weather may be blank (Power BI issues)
            df1.to_csv(out_path+out_file,index=False)                 # ouput prepped csv file
